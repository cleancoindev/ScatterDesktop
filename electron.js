...
const callDeepLink = url => {
  if (global.appShared.ApiWatcher !== null) global.appShared.ApiWatcher(url)
}
  ...
  async initialize(_certs) {
    const socketHandler = socket => {
      ...
      socket.on('message', msg => {
        ...
        const killRequest = () =>
          this.emitSocket(socket, 'api', { id: request.id, result: null })
        ...
        if (
          request.data.hasOwnProperty('payload') &&
          request.data.payload.origin.trim().toLowerCase() === 'Scatter'
        )
          killRequest()
        ...
        if (!origin) origin = requestOrigin
          ...
          return this.emitSocket(socket, 'api', {
            id: request.id,
            result: null
          })
        ...
        switch (type) {
          case 'pair':
            return mainWindow.webContents.send('pair', { request, id })
          case 'rekeyed':
            return this.rekeyPromise.resolve(request)
          case 'api':
            return mainWindow.webContents.send('api', { request, id })
        }
      })
    }
