...
export const getters = {
  ...
  networkTokens: state =>
    state.scatter.settings.networks
      .map(x => {
      ...
      .reduce((acc, token) => {
        const exists = acc.find(
          x => x.unique() === token.unique() && x.chainId === token.chainId
        )
  ...
  // Popups
  popIns: state =>
    state.popups.filter(x => x.displayType === PopupDisplayTypes.POP_IN) || [],
  nextPopIn: state =>
    state.popups.filter(x => x.displayType === PopupDisplayTypes.POP_IN)[0] ||
    null,
  snackbars: state =>
    state.popups.filter(x => x.displayType === PopupDisplayTypes.SNACKBAR) ||
    [],
