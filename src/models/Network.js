...
export default class Network {
  ...
  static fromJson(json) {
    ...
    p.token =
      json.hasOwnProperty('token') && json.token
  ...
  static fromUnique(netString) {
    ...
    if (netString.indexOf(':chain:') > -1)
      return new Network(
        ...
        '',
        blockchain,
        netString.replace(`${blockchain}:chain:`, '')
      )
  ...
  isValid() {
    if (!BlockchainsArray.map(x => x.value).includes(this.blockchain))
      return false
    return (
      this.host.length && this.port.toString().length && this.chainId.length
    )
  }
  ...
  accounts(unique = false) {
    const accounts = StoreService.get().getters.accounts.filter(
      x => x.networkUnique === this.unique()
    )
