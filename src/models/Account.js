...
export default class Account {
  ...
  sendable() {
    return PluginRepository.plugin(this.blockchain()).accountsAreImported()
      ? this.name
      : this.publicKey
  }
  ...
  network() {
    return StoreService.get().state.scatter.settings.networks.find(
      x => x.unique() === this.networkUnique
    )
  }

  keypair() {
    return StoreService.get().state.scatter.keychain.keypairs.find(
      x => x.unique() === this.keypairUnique
    )
  }

  blockchain() {
    if (!this.keypair()) return Blockchains.EOSIO
    return this.keypair().publicKeys.find(x => x.key === this.publicKey)
      .blockchain
  }
  ...
  authorities(thisKeyOnly = true) {
    ...
    return StoreService.get()
      .getters.accounts.filter(
        x =>
          x.identifiable() === this.identifiable() &&
          (!thisKeyOnly || x.keypairUnique === this.keypairUnique)
      )
  ...
  asReturnable() {
    const returnable = PluginRepository.plugin(
      this.blockchain()
    ).returnableAccount(this)
  ...
  tokenCount(systemToken = null) {
    ...
    if (!StoreService.get().state.balances.hasOwnProperty(this.identifiable()))
      return 0
    if (!StoreService.get().state.balances[this.identifiable()]) return 0
    return StoreService.get().state.balances[this.identifiable()].filter(x =>
      !systemToken ? true : x.identifiable() !== systemToken.identifiable()
    ).length
  }
  ...
  balanceFor(token) {
    return this.tokens().find(
      x => x.uniqueWithChain() === token.uniqueWithChain()
    )
  }
  ...
  systemBalance(withSymbol = false) {
    ...
    if (!StoreService.get().state.balances.hasOwnProperty(this.identifiable()))
      return 0
    if (!StoreService.get().state.balances[this.identifiable()]) return 0
    const systemBalance = StoreService.get().state.balances[
      this.identifiable()
    ].find(
      x =>
        Token.fromJson(x).identifiable() ===
        this.network()
          .systemToken()
          .identifiable()
    )
