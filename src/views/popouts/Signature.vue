  ...
  <section>
    <section class="multi-pane popout-window">
      ...
      <section class="main-panel">
        ...
        <section class="participants" v-if="participantAccounts">
          <section v-if="!participantsAsSelector">
            <section
              class="participant"
              v-for="(p, index) in participantAccounts.slice(0,2)"
              :key="index"
            >
              {{p.network().name}} -
              <b>{{p.sendable()}}</b>
            ...
            <figure
              ...
              v-if="participantAccounts.length > 2"
              @click="participantsAsSelector = true"
            >+{{participantAccounts.length}} more accounts</figure>
          </section>
          <Select
            v-else
            bordered="1"
            :options="participantAccounts"
            :parser="x => `${x.network().name} - ${x.sendable()}`"
          />
        </section>
        ...
        <section class="fixed-actions">
          <section
            v-if="isDangerous"
            class="disclaimer less-pad red centered"
            style="margin-bottom:10px;"
          >
            One of the actions included within this transaction is
            <b>dangerous</b>.
          </section>
          ...
          <section class="accept-deny">
            ...
            <!-- ACCEPT TRANSACTION -->
            <Button
              :red="isDangerous || (reputation && reputation.decimal < 0)"
              blue="1"
              v-if="!pinning"
      ...
      <section class="side-panel" v-if="!expanded">
        <section class="messages-scroller">
          <RequiredFields
            v-if="!isArbitrarySignature && (personalFields.length || locationFields.length)"
            :identity="identity"
            :fields="fields"
            ...
            <section :class="{'previous-whitelist':isPreviouslyWhitelisted(message)}">
              ...
              <section class="details contract-action">
                <section
                  class="danger wiggle"
                  v-if="isDangerous"
                  v-tooltip.right="{content:isDangerous, classes:['dangertip']}"
                >
                  <i class="icon-help"></i>
                </section>
                ...
                <figure class="title">
                  <input
                    v-if="whitelisted && !isPreviouslyWhitelisted(message)"
                ...
                <span class="danger-title" v-if="isDangerous">
                  This action is
                  <b>dangerous</b>!
                </span>
              </section>
              ...
              <section v-if="!isCollapsed(message)">
                ...
                >
                  ...
                  <section class="split-inputs">
                    <input
                      v-if="whitelisted && !isPreviouslyWhitelisted(message)"
                      type="checkbox"
                      @change="toggleWhitelistProp(getWhitelist(message), key)"
                    />
                    <figure class="value object" v-if="typeof value === 'object'">
                      <div
                        :ref="hash(JSON.stringify(message)) + key + hash(value)"
                        :v-html="formatJson(value, hash(JSON.stringify(message))+key)"
                      ></div>
                    </figure>
                ...
                <section class="properties" v-if="viewType === VIEW_TYPES.JSON">
                  <div
                    class="value object"
                    :ref="hash(message.data)"
                    :v-html="formatJson(message.data)"
                  ></div>
                </section>
                <section class="properties" v-if="viewType === VIEW_TYPES.RICARDIAN">
                  <figure class="collapsed" v-if="!hasRicardianContract(message)">
                    No Ricardian
                    Contract
                  </figure>
    ...
    <section class="ridl-popup" v-if="showingRidlWarning">
      ...
      <section class="box">
        ...
        <i
          class="link"
          v-for="(reputable, index) in reputation.reputables.filter(x => x.decimal < 0)"
          :key="index"
          @click="openInBrowser(ridlLink(reputable))"
        >
          View
          <b>{{reputable.entity}}</b> on RIDL.
        </i>
      </section>
    </section>
  </section>
...
export default {
  ...
  created() {
    ...
    this.selectedLocation =
      this.selectedIdentity.getLocation() || this.locations[0];
    ...
    setTimeout(async () => {
      ...
      this.reputation = await RIDLService.checkContracts(
        this.payload.network,
        this.messages
      );
      if (this.reputation && this.reputation.decimal < 0)
        this.showingRidlWarning = true;
      this.loadingReputation = false;
    }, 50);
  },
  computed: {
    ...mapState(["scatter", "resources"]),
    ...mapGetters([
      "identity",
      "identities",
      "accounts",
    ...
    viewTypesArray() {
      const hasEos =
        !this.isArbitrarySignature &&
        !!this.payload.participants.find(
          x => Account.fromJson(x).blockchain() === Blockchains.EOSIO
        );
    ...
    missingFields() {
      if (!this.personalFields.length && !this.locationFields.length)
        return false;
      return !this.identity.hasRequiredFields(this.fields);
    },
    isValidIdentity() {
      return this.selectedIdentity.hasRequiredFields(
        this.fields,
        this.clonedLocation
      );
    },
    ...
    cannotSignArbitrary() {
      ...
      return this.payload.messages[0].data.signing
        .split(" ")
        .some(x => x.length > 12);
    },
    isDangerous() {
      if (
        this.messages.find(x => x.code === "eosio" && x.type === "updateauth")
      ) {
  ...
  methods: {
    ...
    reputable(message) {
      ...
      return this.reputation.reputables.find(
        x => x.code === `${message.code}${message.type}`
      );
    },
    ridlLink(reputable) {
    ...
    isCollapsed(message) {
      return this.actionList.find(
        x => x === this.getMessageUnique(message, "collapsed")
      );
    },
    ...
    toggleAction(message, action) {
      ...
      if (this.actionList.includes(unique))
        this.actionList = this.actionList.filter(x => x !== unique);
      else this.actionList.push(unique);
    },
    getAction(message, action) {
      return this.actionList.find(
        x => x === this.getMessageUnique(message, action)
      );
    },
