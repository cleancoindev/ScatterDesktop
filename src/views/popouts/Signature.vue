    ...
    <section>
        <section class="multi-pane popout-window">
            ...
            <section class="main-panel">
                ...
                <figure class="has-more" v-if="limitedMessages.total > 1">
                    {{locale(langKeys.POPOUTS.SIGNATURE.ActionsTotal,limitedMessages.total)}}
                </figure>
                ...
                <section class="participants" v-if="participantAccounts">
                    ...
                    <section v-if="!participantsAsSelector">
                        ...
                        <figure class="more-participants" v-if="participantAccounts.length > 2"
                                @click="participantsAsSelector = true">
                            +{{participantAccounts.length}} more accounts
                        </figure>
                    </section>
                    <Select v-else bordered="1" :options="participantAccounts"
                ...
                <section class="messages-scroller">
                    ...
                    <section class="messages"
                        ...
                        <section class="whitelist-overlay" v-if="isPreviouslyWhitelisted(message)">
                            <section class="box">
                                <figure class="info">{{locale(langKeys.POPOUTS.SIGNATURE.PreviouslyWhitelisted)}}
                                </figure>
                            </section>
                        </section>
                            ...
                            <section v-if="!isCollapsed(message)">
                                ...
                                <section class="properties" v-for="(value,key) in message.data"
                                    ...
                                    <section class="split-inputs">
                                        <input v-if="whitelisted && !isPreviouslyWhitelisted(message)" type="checkbox"
                                               @change="toggleWhitelistProp(getWhitelist(message), key)"/>
                                        <figure class="value object" v-if="typeof value === 'object'">
                                            <div :ref="hash(JSON.stringify(message)) + key + hash(value)"
                                                 :v-html="formatJson(value, hash(JSON.stringify(message))+key)"></div>
                                        </figure>
                                        <figure class="value" v-else>{{value}}</figure>
                                    </section>
                                </section>
                                <section class="properties" v-if="viewType === VIEW_TYPES.JSON">
                                    <div class="value object" :ref="hash(message.data)"
                                         :v-html="formatJson(message.data)"></div>
                                </section>
                                <section class="properties" v-if="viewType === VIEW_TYPES.RICARDIAN">
                                    <figure class="collapsed" v-if="!hasRicardianContract(message)">No Ricardian
                                        Contract
                                    </figure>
        ...
        <section class="ridl-popup" v-if="showingRidlWarning">
            ...
            <section class="box">
                ...
                <i class="link" v-for="reputable in reputation.reputables.filter(x => x.decimal < 0)"
                   @click="openInBrowser(ridlLink(reputable))">View <b>{{reputable.entity}}</b> on RIDL.</i>
            </section>
        </section>
