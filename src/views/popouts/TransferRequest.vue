      ...
      <section class="padded">
        ...
        <section class="boxes">
          <section class="box nested account-selector" @click="selectTokenAndAccount">
            <section>
              <figure class="name">{{$t('TP.NOTOFICATION.POPOUT.SelectAccount')}}</figure>
  ...
  computed: {
    ...
    network() {
      return this.networks.find(
        x => x.unique() === Network.fromJson(this.payload.network).unique()
      );
    },
    blockchain() {
    ...
    token() {
      return Token.fromJson({
        ...
        decimals:
          ...
          PluginRepository.plugin(this.blockchain).defaultDecimals(),
    ...
    validAccounts() {
      return this.accounts
        ...
        .filter(x =>
          [this.network.blockchain].includes(x.blockchain().toLowerCase())
        )
        .filter(
          id =>
            JSON.stringify(id)
              .toLowerCase()
              .indexOf(this.searchTerms.toLowerCase()) > -1
        )
        .reduce((acc, account) => {
          if (
            !acc.find(
              x =>
                account.network().unique() === x.network().unique() &&
                account.sendable() === x.sendable()
            )
          )
            acc.push(account);
    ...
    selectedAccounts() {
      return [this.account].map(x => this.formatAccount(x, false));
    },
    currentAmount() {
