<template>
  <section class="apps">
    <PanelTabs
      :tabs="tabs"
      :state="this.selectedCategory ? this.selectedCategory : state"
      v-on:selected="setState"
    />

    <section
      class="search-icon"
      :class="{'visible':exploreTerms.length}"
      v-if="state === STATES.EXPLORE"
    >
        ...
        <section v-else>
          <section class="categories" v-if="exploreTerms.length">
            <section class="category singular">
              ...
              <section class="apps">
                <router-link
                  :to="{name:RouteNames.APP, params:{applink:app.applink}}"
                  class="app"
                  :key="app.applink"
                ...
                >
                  <figure
                    class="image"
                    :class="{'no-image':!getAppData(app.applink).hasOwnProperty('img')}"
                  >
                    <img
                      v-if="getAppData(app.applink).hasOwnProperty('img')"
                      :src="getAppData(app.applink).img"
                    />
                  </figure>
          ...
          <section class="categories" v-if="!exploreTerms.length">
            <section
              class="category"
              :class="{'singular':selectedCategory}"
              v-for="category in categories"
              v-if="!selectedCategory || selectedCategory === category.type"
            >
              <section class="info" v-if="!selectedCategory">
                <figure class="name">{{category.type}}</figure>
                <figure
                  class="view-all"
                  v-if="category.apps.length > 4"
              ...
              <section class="apps">
                <router-link
                  ...
                  class="app"
                  :key="app.applink"
                  v-for="app in selectedCategory ? category.apps : category.apps.slice(0,10)"
                >
                  <figure
                    class="image"
                    :class="{'no-image':!getAppData(app.applink).hasOwnProperty('img')}"
        ...
        <section class="linked-apps">
          <section class="app" v-for="app in linkedApps">
            ...
            <section class="actions">
              ...
              <Button
                v-if="canOpenApp(app.applink)"
                @click.native="openApp(app.applink)"
                text="Open"
                blue="1"
              />
            </section>
...
export default {
  ...
  computed: {
    ...mapState(["scatter", "dappLogos", "dappData"]),
    ...mapGetters(["permissions", "apps", "accounts"]),
    tabs() {
      let tabs = [{ name: "Explore", state: STATES.EXPLORE }];
      if (this.selectedCategory)
        tabs.splice(1, 0, {
          name: this.selectedCategory,
          state: this.selectedCategory
        });
      if (this.permissions.filter(x => x.isIdentity).length)
        tabs.push({ name: "My Apps", state: STATES.MINE });
      return tabs;
    },
    categories() {
      if (!this.dappData) return {};
      return Object.keys(this.dappData)
        .reduce((acc, key) => {
          const item = this.dappData[key];
          if (!acc.find(x => x.type === item.type))
            acc.push({ type: item.type, apps: [] });
        ...
        .map(cat => {
          ...
          cat.apps = cat.apps.filter(({ applink }) =>
            this.getAppData(applink).hasOwnProperty("img")
          );
          return cat;
        })
        .sort((a, b) => {
      ...
      return Object.keys(this.dappData).reduce((acc, key) => {
        ...
        const found = prop =>
          prop
            .toLowerCase()
            .trim()
            .indexOf(this.exploreTerms.toLowerCase().trim()) > -1;
        if (found(item.applink) || found(item.name) || found(item.description))
          acc.push(item);
        return acc;
      }, []);
    },
    featuredApps() {
      ...
      return this.categories
        .find(x => x.type === this.selectedCategory)
        .apps.slice(0, 3);
    },
    filters() {
      return [
  ...
  methods: {
    ...
    goToApp(app) {
      this.$router.push({
        name: this.RouteNames.APP,
        params: { applink: app.applink }
      });
    },
