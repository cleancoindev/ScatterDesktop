...
/* 格式化日期 */
Utils.formatDate = function (datetime, lang, simple = false) {
    let tempDate = (new Date()).getTime();
    let result = this.stringifyDate(datetime, lang, simple);
    ...
    if (msg.type === 'custom') {
        try {
            ...
            switch (cnt.type) {
                ...
                // 红包领取的通知
                case 2:
                    return `${msg.fromNick}领取了红包`;
                // DApp
                case 3:
                    return '[DAPP消息,请到手机端查看]';
                // 文章链接
                case 4:
                    return '[文章消息]';
                // 转账
                ...
                // 表情图
                case 6:
                    return '[表情图消息,请到手机端查看]';
                default:
                    return '[自定义消息,请到手机端查看]';
            }
        } catch (e) {
