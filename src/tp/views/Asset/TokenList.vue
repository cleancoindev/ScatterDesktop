    ...
    <section class="token-list">
        ...
        <section class="tokens">
            <section class="single-asset"
                     ...
                     v-for="token in sortedBalances" @click="selectToken(token)">

                <section class="row">
                    <section class="token-symbol">
                        <div class="symbol"
                             :class="[{'iconed':token.symbolClass(), 'small':token && token.symbol.length >= 4, 'unusable':!!token.unusable}, token.symbolClass()]"></div>
                        <figure class="title ft-24">{{token.symbol}}</figure>
                    </section>
                    <section class="token-value ft-20 " v-if="token.amount">
                        <figure class="value c-232538">{{formatNumber(token.amount, true)}}</figure>
                        <figure class="fiat c-232538" v-if="token.fiatBalance() && parseFloat(token.fiatBalance())">
                            ≈ {{fiatSymbol(displayCurrency)}}{{formatNumber(token.fiatBalance(false), true)}}
                        </figure>
                    </section>
                </section>
...
<!--                        <figure class="secondary" v-if="token.baseTokenPrice() && parseFloat(token.baseTokenPrice())">-->
<!--                            {{formatNumber(token.baseTokenPrice(), true)}}-->
<!--                        </figure>-->
<!--                        <figure class="secondary" v-if="token.fiatPrice() && parseFloat(token.fiatPrice())">-->
<!--                            {{formatNumber(token.fiatPrice(), true)}}-->
<!--                        </figure>-->
<!--                    </section>-->

<!--                    <section class="token-change" v-if="change(token).perc">-->
<!--                        <figure class="change-value" :class="{'red':!change(token).plus}" v-if="!token.unusable">-->
<!--                            {{change(token).perc}}-->
<!--                        </figure>-->
<!--                    </section>-->
                </section>
