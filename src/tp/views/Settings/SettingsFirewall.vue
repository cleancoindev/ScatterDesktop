...
<!-- 
        ...
        <section class="action-box top-pad" style="background: #fff;border: 0">
            <label>{{$t('TP.FIREWALL.TEXT1')}}</label>
            <p>{{$t('TP.FIREWALL.TEXT2')}}</p>
            <br>
            <br>

            <label>{{$t('TP.FIREWALL.TEXT3')}}</label>
            <section class="split-inputs">
                ...
                <Input :placeholder="$t('TP.GENERIC.Contract')" :text="newContract" v-on:changed="x => newContract = x.trim().toLowerCase()" />
                <Input :placeholder="$t('TP.GENERIC.Action')" :text="newAction" v-on:changed="x => newAction = x.trim().toLowerCase()" />
            </section>
            <br>
            ...
            <br>
            <hr>
            <label>{{$t('TP.FIREWALL.TEXT4')}}</label>
            <br>
                ...
                <section class="blacklist" v-for="(actions, contract) in blacklistActions">
                    ...
                    <figure class="actions">
                        <figure class="action" v-for="action in actions">
                            ...
                            <Button @click.native="removeBlacklist(contract, action)" class="remove" :text="$t('TP.GENERIC.Remove')" small="1" />
                        </figure>
                    </figure>
                </section>
            </section>
