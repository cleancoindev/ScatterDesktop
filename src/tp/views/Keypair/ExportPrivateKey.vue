<template>
  <section class="pop-in text-center">
    ...
    <!--					&lt;!&ndash; EXPORT KEY &ndash;&gt;-->
    <!--					<section class="type" @click="state = STATES.KEY">-->
    <!--						<figure class="badge icon-key"></figure>-->
    <!--						<figure class="name">{{$t(langKeys.KEYPAIR.EXPORT.SELECT.KeyTitle)}}</figure>-->
    <!--						<p>{{$t(langKeys.KEYPAIR.EXPORT.SELECT.KeyDescription)}}</p>-->
    <!--					</section>-->

    <!--					&lt;!&ndash; EXPORT QR &ndash;&gt;-->
    <!--					<section class="type" @click="createQR">-->
    <!--						<figure class="badge icon-qrcode"></figure>-->
    <!--						<figure class="name">{{$t(langKeys.KEYPAIR.EXPORT.SELECT.QrTitle)}}</figure>-->
    <!--						<p>{{$t(langKeys.KEYPAIR.EXPORT.SELECT.QrDescription)}}</p>-->
    <!--					</section>-->
    ...
    <!-- EXPORT AS TEXT KEY -->
    <section class="export-text text-center">
      <h1>{{$t('TP.GENERIC.Export')}}{{$t('TP.GENERIC.PrivateKey')}}</h1>
      <section class="split-inputs">
        <Input style="flex:1; margin-bottom:0;" :text="privateKey" />
        <Button :text="$t('TP.GENERIC.Copy')" @click.native="copyPrivateKey" />
      </section>
    </section>
    ...
    <!--					<h1>{{$t(langKeys.KEYPAIR.EXPORT.QR.Title)}}</h1>-->
...
import ElectronHelpers, {
  ipcAsync,
  remote
} from "../../../util/ElectronHelpers";
...
export default {
  ...
  computed: {
    ...
    keypair() {
      ...
      return this.keypairs.find(keypair => keypair.id === this.$route.query.id);
    }
  },
  ...
  methods: {
    ...
    async getPrivateKey() {
      ...
      return Crypto.bufferToPrivateKey(
        this.keypair.privateKey,
        this.keypair.enabledKey().blockchain
      );
    },
    ...
    screenshot() {
      ...
      setTimeout(() => {
        let location = remote.dialog.showOpenDialog({
        ...
        remote.getCurrentWindow().capturePage(img => {
          remote.require("fs").writeFile(filename, img.toJPEG(99), saved => {
            PopupService.push(
              Popup.snackbar(
                this.$t(this.langKeys.SNACKBARS.SavedImage),
                "check"
              )
            );
