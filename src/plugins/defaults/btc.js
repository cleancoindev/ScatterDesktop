...
const SELECTED_NETWORK =
  SELECTED_CHAIN === 0 ? bitcoin.networks.bitcoin : bitcoin.networks.testnet
...
export default class BTC extends Plugin {
  constructor() {
    super(Blockchains.BTC, PluginTypes.BLOCKCHAIN_SUPPORT)
  }
  ...
  returnableAccount(account) {
    return { address: account.publicKey, blockchain: Blockchains.BTC }
  }
  ...
  isEndorsedNetwork(network) {
    ...
    return (
      network.blockchain === Blockchains.BTC &&
  ...
  privateToPublic(privateKey) {
    const pubkey = (() => {
      if (typeof privateKey === 'string')
        return bitcoin.ECPair.fromWIF(privateKey, SELECTED_NETWORK)
      else
  ...
  validPrivateKey(privateKey) {
    return (
      (typeof privateKey === 'string'
        ? privateKey
        : this.bufferToHexPrivate(privateKey)
      ).length === 52
    )
  }
  ...
  async balanceFor(account) {
    return explorer
      ...
      .then(res => {
        ...
        token.amount =
          parseInt(res[account.publicKey].final_balance) / 100000000
        return token
      })
      .catch(() => {
        PopupService.push(
          Popup.snackbar(
            `There was a problem loading balances for ${account.publicKey}`
          )
        )
        this.defaultToken()
      })
  ...
  async transfer({ account, to, amount, promptForSignature = true }) {
    ...
    try {
      return new Promise(async (resolve, reject) => {
        ...
        const utxos = await explorer
          .getUnspentOutputs(account.publicKey)
          .then(x => x.unspent_outputs)
          .catch(() => null)
        if (!utxos)
          return resolve({
            error: `There was a problem loading UTXOs for ${account.publicKey}`
          })
        ...
        if (change < 0)
          return resolve({
            error: `Insufficient BTC: ${inputs}. (Possibly related to utxos, contact support)`
          })
  ...
  async signer(
    ...
    try {
      ...
      const key = bitcoin.ECPair.fromPrivateKey(Buffer.from(basePrivateKey), {
        network: SELECTED_NETWORK
      })
      const txb = bitcoin.TransactionBuilder.fromTransaction(
        bitcoin.Transaction.fromHex(transaction),
        SELECTED_NETWORK
      )
