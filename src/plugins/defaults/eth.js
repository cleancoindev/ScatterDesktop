...
const getCachedInstance = (network, wallet = null) => {
  ...
  if (cachedInstances.hasOwnProperty(key)) return cachedInstances[key]
    ...
    const rpcUrl =
      network.host === 'ethnodes.get-scatter.com'
        ? 'https://commonly-classic-katydid.quiknode.io/d0bf98e7-a866-43d4-ac71-2397fd1b3aba/dQsznyrZRg2dr4DQJNPDgw==/'
        : network.fullhost()
...
export default class ETH extends Plugin {
  ...
  returnableAccount(account) {
    return { address: account.publicKey, blockchain: Blockchains.ETH }
  }
  ...
  isEndorsedNetwork(network) {
    ...
    return (
      network.blockchain === Blockchains.ETH &&
  ...
  privateToPublic(privateKey) {
    return ethUtil.addHexPrefix(
      ethUtil.privateToAddress(toBuffer(privateKey)).toString('hex')
    )
  }

  validPrivateKey(privateKey) {
    return (
      privateKey.length === 64 && ethUtil.isValidPrivate(toBuffer(privateKey))
    )
  }
      ...
      new Promise(async resolve => {
        if (token.uniqueWithChain() === this.defaultToken().uniqueWithChain()) {
          balance = await web3.utils.fromWei(
            await web3.eth.getBalance(account.publicKey)
          )
        } else {
          const contract = new web3.eth.Contract(erc20abi, token.contract)
          try {
            balance = TokenService.formatAmount(
              await contract.methods.balanceOf(account.sendable()).call(),
              token,
              true
            )
          } catch (e) {
  ...
  actionParticipants(payload) {
    return ObjectHelpers.flatten(
      payload.messages.map(message => message.authorization)
    )
  }
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        if (isEth) {
          ...
          web3.eth
            .sendTransaction({ from: account.publicKey, to, value })
            .on('transactionHash', transactionHash =>
              finished({ transactionHash })
            )
  ...
  async signerWithPopup(payload, account, rejector, token = null) {
    return new Promise(async resolve => {
      payload.messages = await this.requestParser(
        payload.transaction,
  ...
  async requestParser(transaction, abi, token = null) {
    ...
    if (abi) {
      methodABI = abi.find(
        method => transaction.data.indexOf(method.signature) !== -1
      )
      if (!methodABI)
    ...
    const valueParam = data.hasOwnProperty('value')
      ? 'value'
    ...
    if (valueParam) {
      ...
      if (typeof data[valueParam] === 'object') {
        const objParam = data[valueParam].hasOwnProperty('hex')
          ? 'hex'
          : data[valueParam].hasOwnProperty('_hex')
          ? '_hex'
          : null
