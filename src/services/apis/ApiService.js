...
export default class ApiService {
  static async handleDeepLink(deepLink) {
    ...
    if (type === 'transfer') {
      ...
      if (!chainId || !chainId.length)
        chainId = PluginRepository.plugin(blockchain).getEndorsedNetwork()
          .chainId
      if (!memo) memo = ''
      ...
      const network = StoreService.get().state.scatter.settings.networks.find(
        x => x.blockchain === blockchain && x.chainId === chainId
      )
      if (!network) return
  ...
  static async loginHandler(request, loginAll) {
    return new Promise(resolve => {
      ...
      const requiredNetworks = fields.accounts
      ...
      if (!loginAll && requiredNetworks.length > 1) {
        return resolve({
          ...
          result: Error.signatureError(
            'too_many_accounts',
            'To login more than one account you must use the `getAllAccounts()` API method.'
          )
        })
      }

      const existingNetworks = StoreService.get().state.scatter.settings.networks.filter(
        x => requiredNetworks.includes(x.unique())
      )
      if (existingNetworks.length !== requiredNetworks.length) {
        return resolve({ id: request.id, result: Error.noNetwork() })
      }

      const availableAccounts = existingNetworks
        .map(x => x.accounts(true))
        .reduce((acc, accounts) => {
      ...
      if (possibleId) {
        const samePersonal = fields.personal.every(
          key =>
            possibleId.hasOwnProperty('personal') &&
            possibleId.personal.hasOwnProperty(key)
        )
        const sameLocation = fields.location.every(
          key =>
            possibleId.hasOwnProperty('location') &&
            possibleId.location.hasOwnProperty(key)
        )

        let sameAccounts = true
        if (loginAll && availableAccounts.length !== possibleId.accounts.length)
          sameAccounts = false
        else if (!loginAll && possibleId.accounts.length > 1)
          sameAccounts = false

        if (samePersonal && sameLocation && sameAccounts)
          return resolve({ id: request.id, result: possibleId })
      }

      PopupService.push(
        Popup.popout(request, async ({ result }) => {
          if (!result)
            return resolve({
              id: request.id,
              result: Error.signatureError(
          ...
          // const identity = Identity.fromJson(result.identity);
          const identity = StoreService.get().state.scatter.keychain.identities.find(
            x => x.id === result.identity.id
          )
          await identity.setAsLastUsed()

          const location = LocationInformation.fromJson(result.location)
          const accounts = loginAll
            ? availableAccounts
            : (result.accounts || []).map(x => Account.fromJson(x))

          await PermissionService.addIdentityOriginPermission(
            identity,
            accounts,
            fields,
            origin
          )
          const returnableIdentity = identity.asOnlyRequiredFields(
            fields,
            location
          )
          returnableIdentity.accounts = accounts.map(x => x.asReturnable())

          if (!loginAll && accounts.length)
  ...
  static async [Actions.SIGN](request) {
    ...
    return new Promise(async resolve => {
      ...
      const possibleId = PermissionService.identityFromPermissions(
        origin,
        false
      )
      if (!possibleId)
        return resolve({ id: request.id, result: Error.identityMissing() })
      ...
      const network = StoreService.get().state.scatter.settings.networks.find(
        x => x.unique() === Network.fromJson(payload.network).unique()
      )
      if (!network)
        return resolve({ id: request.id, result: Error.noNetwork() })
      ...
      switch (blockchain) {
        case Blockchains.EOSIO:
          payload.messages = await plugin.requestParser(payload, network)
          break
        case Blockchains.ETH:
        case Blockchains.TRX:
          payload.messages = await plugin.requestParser(
            payload,
            payload.hasOwnProperty('abi') ? payload.abi : null
          )
          break
      }
      ...
      const blacklisted = payload.messages
        ...
        .filter(actionTag =>
          StoreService.get().state.scatter.settings.isActionBlacklisted(
            actionTag
          )
        )
      ...
      const fillIdentity = () =>
        (identity = StoreService.get().state.scatter.keychain.identities.find(
          x => x.publicKey === possibleId.publicKey
        ))
      fillIdentity()
      ...
      const existingApp = StoreService.get().state.scatter.keychain.findApp(
        origin
      )

      const hasHardwareKeys = participants.some(x =>
  ...
  static async [Actions.SIGN_ARBITRARY](request, identityKey = null) {
    ...
    return new Promise(async resolve => {
      ...
      if (data.split(' ').some(x => x.length > 12))
        return resolve({
          id: request.id,
          result: Error.malicious(
            'You can not sign strings where any of the words are over 12 characters.'
          )
        })
      ...
      const blockchain = keypair.publicKeys.find(x => x.key === publicKey)
        .blockchain
      ...
      PopupService.push(
        Popup.popout(Object.assign(request, {}), async ({ result }) => {
          ...
          resolve({
            id: request.id,
            result: await plugin.signer(payload, publicKey, true, false)
          })
        })
      )
    })
  ...
  static async [Actions.TRANSFER](request) {
    ...
    return new Promise(resolve => {
      ...
      network = StoreService.get().state.scatter.settings.networks.find(
        x => x.unique() === Network.fromJson(network).unique()
      )
      if (!network)
        return resolve({ id: request.id, result: Error.noNetwork() })
      ...
      PopupService.push(
        Popup.popout(request, async ({ result }) => {
          ...
          const token = Token.fromJson({
            ...
            decimals:
              options.decimals ||
              PluginRepository.plugin(network.blockchain).defaultDecimals(),
            chainId: account.network().chainId
          })
          const sent = await PluginRepository.plugin(network.blockchain)
            .transfer({
              account,
              to,
  ...
  static async [Actions.GET_PUBLIC_KEY](request) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (
        !BlockchainsArray.map(x => x.value).includes(request.payload.blockchain)
      )
        return badResult('no such blockchain')

      PopupService.push(
        Popup.popout(request, async ({ result }) => {
          if (!result)
            return resolve({ id: request.id, result: Error.rejected() })

          const keypair = Keypair.fromJson(result.keypair)
          const publicKey = keypair.publicKeys.find(
            x => x.blockchain === request.payload.blockchain
          ).key

          if (result.isNew) {
            await KeyPairService.saveKeyPair(keypair)
            router.push({
              name: RouteNames.KEYPAIR,
              params: { id: keypair.id }
            })
      ...
      if (kyc && kyc.length) {
        if (kyc.indexOf('::') === -1)
          return resolve({
            ...
            result: Error.signatureError(
              'invalid_kyc',
              'KYC properties must be formatted as: domain::hash'
            )
          })
        ...
        Popup.popout(Object.assign(request, {}), async ({ result }) => {
          if (!result)
            return resolve({
              ...
              result: Error.signatureError(
                'update_rejected',
                'User rejected the update request'
              )
            })

          const scatter = StoreService.get().state.scatter.clone()
          const identity = scatter.keychain.identities.find(
            x => x.id === possibleId.id
          )
          ...
          resolve({
            id: request.id,
            result: PermissionService.identityFromPermissions(origin, true)
          })
        })
      )
    })
  ...
  static async [Actions.IDENTITY_FROM_PERMISSIONS](request) {
    const result = PermissionService.identityFromPermissions(
  ...
  static async [Actions.GET_AVATAR](request) {
    ...
    return {
      id: request.id,
      result: StoreService.get().state.scatter.keychain.avatars[possibleId.id]
    }
  }

  static async [Actions.AUTHENTICATE](request) {
    return new Promise(async resolve => {
      const identity = PermissionService.identityFromPermissions(
        request.payload.origin
      )
      if (!identity)
        return resolve({ id: request.id, result: Error.identityMissing() })
      ...
      const nonceError = new Error(
        'invalid_nonce',
        'You must provide a 12 character nonce for authentication'
      )
      if (!request.payload.hasOwnProperty('nonce'))
        return resolve({ id: request, result: nonceError })
      if (request.payload.nonce.length !== 12)
        return resolve({ id: request, result: nonceError })

      const publicKey =
        request.payload.hasOwnProperty('publicKey') &&
        request.payload.publicKey &&
        request.payload.publicKey.length
          ? request.payload.publicKey
          : identity.publicKey

      const keypair = KeyPairService.getKeyPairFromPublicKey(publicKey)
      if (!keypair)
        return resolve({ id: request.id, result: Error.noKeypair() })

      const isHash =
        request.payload.hasOwnProperty('data') &&
        request.payload.data &&
        request.payload.data.length
  ...
  static async [Actions.LINK_ACCOUNT](request) {
    return new Promise(async (resolve, reject) => {
      ...
      const keypair = scatter.keychain.keypairs.find(x =>
        x.publicKeys.some(y => y.key === account.publicKey)
      )
      if (!keypair)
        return resolve({ id: request.id, result: Error.noKeypair() })
      ...
      if (
        scatter.keychain.accounts.find(
          x =>
            x.fromOrigin === origin && x.createdAt > +new Date() - 3600 * 1000
        )
      )
        return resolve({
          id: request.id,
          result: new Error(
            'link_account_timeout',
            'You can only add 1 account every hour.'
          )
        })
  ...
  static async [Actions.SUGGEST_NETWORK](request) {
    return new Promise(async resolve => {
      ...
      if (network.hasOwnProperty('token') && network.token) {
        ...
        network.token.name = network.token.name.length
          ? network.token.name
      ...
      if (!network.isValid())
        return resolve({
          ...
          result: new Error(
            'bad_network',
            'The network being suggested is invalid'
          )
        })

      if (
        StoreService.get().state.scatter.settings.networks.find(
      ...
      if (
        StoreService.get().state.scatter.settings.networks.find(
          x =>
            x.fromOrigin === request.payload.origin &&
            x.createdAt > +new Date() - 3600 * 12 * 1000
        )
      )
        return resolve({
          id: request.id,
          result: new Error(
            'network_timeout',
            'You can only add 1 network every 12 hours.'
          )
        })

      // All applications can only add 5 networks every 12 hours.
      if (
        StoreService.get().state.scatter.settings.networks.filter(
          x => x.createdAt > +new Date() - 3600 * 12 * 1000
        ) > 5
      )
        return resolve({
          id: request.id,
          result: new Error(
            'network_timeout',
            'Too many networks were added over the past 12 hours'
          )
        })
    ...
    return new Promise(async resolve => {
      ...
      if (!token.isValid())
        return resolve({
          ...
          result: new Error(
            'invalid_token',
            'The token specified is not a valid token object.'
          )
        })

      // Applications can only add one token every 12 hours.
      if (
        StoreService.get().state.scatter.settings.tokens.filter(
          x =>
            x.fromOrigin === request.payload.origin &&
            x.createdAt > +new Date() - 3600 * 12 * 1000
        ).length > 5
        ...
        return resolve({
          ...
          result: new Error(
            'token_timeout',
            'You can only add up to 5 tokens every 12 hours.'
          )
        })
      ...
      if (
        ...
        return resolve({
          ...
          result: new Error(
            'token_timeout',
            'Too many tokens were added over the past 12 hours.'
          )
        })
      ...
      if (exists)
        return resolve({
          ...
          result: new Error(
            'token_exists',
            'The user already has this token in their Scatter.'
          )
        })
  ...
  static async [Actions.HAS_ACCOUNT_FOR](request) {
    return new Promise(resolve => {
      ...
      resolve({
        id: request.id,
        result: !!StoreService.get().state.scatter.keychain.accounts.find(
          x => x.networkUnique === network.unique()
        )
      })
    })
  }
  ...
  static async [Actions.GET_VERSION](request) {
    return new Promise(resolve => {
      resolve({
        id: request.id,
        result: StoreService.get().state.scatter.meta.version
      })
    })
  }
}
